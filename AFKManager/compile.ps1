...
# Stage lang files used by plugin translations
if (Test-Path $langSource) {
    Copy-Item -Path $langSource -Destination $pluginTarget -Recurse -Force
} else {
    Write-Host '[WARN] No lang directory found in repository root.'
}
